...
void test_pid_state_save_load() {
    ...
    saveState.lastError = -67.89f;
    saveState.output = 50.0f;
    saveState.lastUpdateTime = millis();
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.01f, saveState.lastError, loadState.lastError);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, saveState.output, loadState.output);
    TEST_ASSERT_EQUAL(saveState.lastUpdateTime, loadState.lastUpdateTime);
}
...
void test_counter_operations() {
    ...
    bool setResult = storage->setCounter(COUNTER_BURNER_STARTS, 100);
    TEST_ASSERT_TRUE(setResult);

    // Verify value
    uint32_t value = storage->getCounter(COUNTER_BURNER_STARTS);
    TEST_ASSERT_EQUAL(100, value);

    // Increment
    bool incResult = storage->incrementCounter(COUNTER_BURNER_STARTS);
    TEST_ASSERT_TRUE(incResult);

    // Verify incremented value
    value = storage->getCounter(COUNTER_BURNER_STARTS);
...
void test_runtime_hours() {
    ...
    bool result = storage->updateRuntimeHours(RUNTIME_BURNER, 123.5f);
    TEST_ASSERT_TRUE(result);

    // Read back
    float hours = storage->getRuntimeHours(RUNTIME_BURNER);
...
void test_event_logging() {
    ...
    (void)storage->clearEvents();   // result intentionally unused in test setup

    // Log some events
    bool result1 = storage->logEvent(EVENT_SYSTEM, 0);
    bool result2 = storage->logEvent(EVENT_WARNING, 850);  // 85.0°C
    bool result3 = storage->logEvent(EVENT_ERROR, 123);
...
void test_event_retrieval() {
    ...
    (void)storage->clearEvents();   // result intentionally unused in test setup
    (void)storage->logEvent(EVENT_SYSTEM, 1);   // result intentionally unused in test setup
    (void)storage->logEvent(EVENT_SYSTEM, 2);   // result intentionally unused in test setup
    (void)storage->logEvent(EVENT_SYSTEM, 3);   // result intentionally unused in test setup
    ...
    TEST_ASSERT_EQUAL(EVENT_SYSTEM, buffer[0].type);
}
...
void test_system_state() {
    ...
    saveState.operatingMode = 2;
    saveState.activeErrors = 0x0003;
    saveState.lastShutdownReason = 0;
    saveState.uptimeAtShutdown = millis();
    ...
    // Verify
    TEST_ASSERT_EQUAL(saveState.operatingMode, loadState.operatingMode);
    TEST_ASSERT_EQUAL(saveState.activeErrors, loadState.activeErrors);
    TEST_ASSERT_EQUAL(saveState.uptimeAtShutdown, loadState.uptimeAtShutdown);
}
...
void test_error_logging() {
    ...
    (void)storage->clearErrors();   // result intentionally unused in test setup

...
void setup() {
    ...
    // Log system startup
    framOk(fram.logEvent(rtstorage::EVENT_SYSTEM, 0x01), "logEvent");  // System start
...
void loop() {
    ...
    rtstorage::Temperature_t flowTemp = 650 + random(-20, 20);      // 65°C ± 2°C
    rtstorage::Temperature_t returnTemp = 450 + random(-20, 20);    // 45°C ± 2°C
    rtstorage::Temperature_t outsideTemp = 100 + random(-10, 10);   // 10°C ± 1°C
    rtstorage::Temperature_t roomTemp = 200 + random(-5, 5);        // 20°C ± 0.5°C
    ...
    if (millis() - lastPidSave > 5000) {
        ...
        framOk(fram.savePIDState(PID_HEATING, pidState), "savePIDState");
        ...
        pidState.lastUpdateTime = millis();
        framOk(fram.savePIDState(PID_WATER, pidState), "savePIDState");
        
        // Update runtime hours
        float currentHours = fram.getRuntimeHours(rtstorage::RUNTIME_TOTAL);
        framOk(fram.updateRuntimeHours(rtstorage::RUNTIME_TOTAL, currentHours + (5.0 / 3600.0)), "updateRuntimeHours");
        
        if (currentMode == MODE_HEATING || currentMode == MODE_BOTH) {
            currentHours = fram.getRuntimeHours(rtstorage::RUNTIME_HEATING);
            framOk(fram.updateRuntimeHours(rtstorage::RUNTIME_HEATING, currentHours + (5.0 / 3600.0)), "updateRuntimeHours");
        }
    }
    ...
    if (millis() - lastTempLog > 30000) {
        ...
        framOk(fram.recordTemperature(SENSOR_BOILER_FLOW, flowTemp), "recordTemperature");
        framOk(fram.recordTemperature(SENSOR_BOILER_RETURN, returnTemp), "recordTemperature");
        framOk(fram.recordTemperature(SENSOR_OUTSIDE_TEMP, outsideTemp), "recordTemperature");
        framOk(fram.recordTemperature(SENSOR_ROOM_TEMP, roomTemp), "recordTemperature");
...
void printStatus(rtstorage::Temperature_t flow, rtstorage::Temperature_t ret,
    ...
    float heatingHours = fram.getRuntimeHours(rtstorage::RUNTIME_HEATING);
    float waterHours = fram.getRuntimeHours(rtstorage::RUNTIME_WATER);
...
void showStatistics() {
    ...
    Serial.printf("Heating pump starts: %lu\n", fram.getCounter(rtstorage::COUNTER_HEATING_PUMP_STARTS));
    Serial.printf("Water pump starts: %lu\n", fram.getCounter(rtstorage::COUNTER_WATER_PUMP_STARTS));
    Serial.printf("Error count: %lu\n", fram.getCounter(rtstorage::COUNTER_ERROR_COUNT));
    
    // Show runtime hours
    Serial.printf("Total runtime: %.2f hours\n", fram.getRuntimeHours(rtstorage::RUNTIME_TOTAL));
    Serial.printf("Heating runtime: %.2f hours\n", fram.getRuntimeHours(rtstorage::RUNTIME_HEATING));
    Serial.printf("Water runtime: %.2f hours\n", fram.getRuntimeHours(rtstorage::RUNTIME_WATER));
    Serial.printf("Burner runtime: %.2f hours\n", fram.getRuntimeHours(rtstorage::RUNTIME_BURNER));
...
void setMode(OperatingMode mode) {
    if (mode != currentMode) {
        // Log mode change
        framOk(fram.logEvent(rtstorage::EVENT_STATE_CHANGE, 
                     (currentMode << 8) | mode), "logEvent");
        ...
        framOk(fram.saveSystemState(state), "saveSystemState");
        
        // Update pump counters
        if (mode == MODE_HEATING || mode == MODE_BOTH) {
            framOk(fram.incrementCounter(rtstorage::COUNTER_HEATING_PUMP_STARTS), "incrementCounter");
        }
        if (mode == MODE_WATER || mode == MODE_BOTH) {
            framOk(fram.incrementCounter(rtstorage::COUNTER_WATER_PUMP_STARTS), "incrementCounter");
        }
    }
}
...
void showEventLog() {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        switch (events[i].type) {
            case rtstorage::EVENT_ERROR:
                Serial.print(F("ERROR"));
                break;
            case rtstorage::EVENT_WARNING:
                Serial.print(F("WARNING"));
                break;
            case rtstorage::EVENT_STATE_CHANGE:
                Serial.print(F("STATE"));
                break;
            case rtstorage::EVENT_USER_ACTION:
                Serial.print(F("USER"));
                break;
            case rtstorage::EVENT_SYSTEM:

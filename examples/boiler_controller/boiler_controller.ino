...
void setup() {
    ...
    // Log system startup
    fram.logEvent(rtstorage::EVENT_SYSTEM, 0x01);  // System start
...
void loop() {
    ...
    if (millis() - lastPidSave > 5000) {
        ...
        fram.updateRuntimeHours(rtstorage::RUNTIME_TOTAL, currentHours + (5.0 / 3600.0));
        
        if (currentMode == MODE_HEATING || currentMode == MODE_BOTH) {
            currentHours = fram.getRuntimeHours(rtstorage::RUNTIME_HEATING);
            fram.updateRuntimeHours(rtstorage::RUNTIME_HEATING, currentHours + (5.0 / 3600.0));
        }
    }
...
void setMode(OperatingMode mode) {
    if (mode != currentMode) {
        // Log mode change
        fram.logEvent(rtstorage::EVENT_STATE_CHANGE, 
                     (currentMode << 8) | mode);
        ...
        if (mode == MODE_WATER || mode == MODE_BOTH) {
            fram.incrementCounter(rtstorage::COUNTER_WATER_PUMP_STARTS);
        }
    }
}

...
void setup() {
    ...
    if (!storage.begin(Wire, FRAM_I2C_ADDRESS)) {
        ...
        framOk(storage.logCriticalError(0xFFFF, "FRAM init failed", "Setup"), "logCriticalError");
        delay(5000);
        esp_restart();
    }
    ...
    // Log system startup
    framOk(storage.logEvent(rtstorage::EVENT_SYSTEM, 0x01), "logEvent");  // System start
    framOk(storage.incrementCounter(rtstorage::COUNTER_SYSTEM_RESTARTS), "incrementCounter");
    ...
    if (!boilerController.begin(&storage, &i2cMutex)) {
        ...
        framOk(storage.logCriticalError(0xFFFE, "Controller init failed", "Setup"), "logCriticalError");
        delay(5000);
        esp_restart();
    }
    ...
    while (true) {
        ...
        if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(100))) {
            ...
            if (millis() - lastTempLog > 60000) {  // Every minute
                ...
                framOk(storage.recordTemperature(0, (int16_t)(flowTemp * 10)), "recordTemperature");
                framOk(storage.recordTemperature(1, (int16_t)(returnTemp * 10)), "recordTemperature");
                framOk(storage.recordTemperature(2, (int16_t)(outsideTemp * 10)), "recordTemperature");
                framOk(storage.recordTemperature(3, (int16_t)(roomTemp * 10)), "recordTemperature");
            }
        }
    ...
    while (true) {
        ...
        if (boilerController.hasError()) {
            ...
            if (boilerController.isCriticalError()) {
                framOk(storage.logCriticalError(errorCode, errorMsg, "Control"), "logCriticalError");
            } else {
                framOk(storage.logError(errorCode, errorMsg, "Control"), "logError");
            }
        }
    ...
    while (true) {
        ...
        for (uint8_t i = 0; i < 4; i++) {
            ...
            if (boilerController.getPIDState(i, pidState)) {
                framOk(storage.savePIDState(i, pidState), "savePIDState");
            }
        }
        
        // Update runtime hours
        float currentHours = storage.getRuntimeHours(rtstorage::RUNTIME_TOTAL);
        framOk(storage.updateRuntimeHours(rtstorage::RUNTIME_TOTAL, 
            currentHours + (5.0 / 3600.0)), "updateRuntimeHours");  // Add 5 seconds
        ...
        if (mode == 1 || mode == 3) {  // Heating or Both
            ...
            framOk(storage.updateRuntimeHours(rtstorage::RUNTIME_HEATING, 
                currentHours + (5.0 / 3600.0)), "updateRuntimeHours");
        }
        if (mode == 2 || mode == 3) {  // Water or Both
            currentHours = storage.getRuntimeHours(rtstorage::RUNTIME_WATER);
            framOk(storage.updateRuntimeHours(rtstorage::RUNTIME_WATER, 
                currentHours + (5.0 / 3600.0)), "updateRuntimeHours");
        }
        ...
        framOk(storage.saveSystemState(sysState), "saveSystemState");
    ...
    if (Serial.available()) {
        ...
        switch (cmd) {
            case '0':  // Off
                ...
                framOk(storage.logEvent(rtstorage::EVENT_USER_ACTION, 0), "logEvent");
                Serial.println(F("Mode: OFF"));
                break;
                
            case '1':  // Heating
                boilerController.setOperatingMode(1);
                framOk(storage.logEvent(rtstorage::EVENT_USER_ACTION, 1), "logEvent");
                framOk(storage.incrementCounter(rtstorage::COUNTER_HEATING_PUMP_STARTS), "incrementCounter");
                Serial.println(F("Mode: HEATING"));
                break;
                
            case '2':  // Hot Water
                boilerController.setOperatingMode(2);
                framOk(storage.logEvent(rtstorage::EVENT_USER_ACTION, 2), "logEvent");
                framOk(storage.incrementCounter(rtstorage::COUNTER_WATER_PUMP_STARTS), "incrementCounter");
                Serial.println(F("Mode: HOT WATER"));
                break;
                
            case '3':  // Both
                boilerController.setOperatingMode(3);
                framOk(storage.logEvent(rtstorage::EVENT_USER_ACTION, 3), "logEvent");
                framOk(storage.incrementCounter(rtstorage::COUNTER_HEATING_PUMP_STARTS), "incrementCounter");
                framOk(storage.incrementCounter(rtstorage::COUNTER_WATER_PUMP_STARTS), "incrementCounter");
            ...
            case 'c':  // Clear errors
                framOk(storage.clearErrors(), "clearErrors");
                Serial.println(F("Errors cleared"));
                break;
                
            case 'r':  // Reset FRAM
                Serial.println(F("Resetting FRAM..."));
                framOk(storage.reset(), "reset");

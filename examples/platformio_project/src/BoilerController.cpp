        ...
        case MODE_BOTH: {
            ...
            if (!_burnerOn && _flowTemp < targetFlow - 5.0f) {
                ...
                if (now - _burnerStopTime >= BURNER_MIN_OFF_TIME) {
                    ...
                    framOk(_storage->incrementCounter(rtstorage::COUNTER_BURNER_STARTS), "incrementCounter");
                    framOk(_storage->logEvent(rtstorage::EVENT_STATE_CHANGE, 0x0100), "logEvent");  // Burner on
                }
                ...
                if (now - _burnerStartTime >= BURNER_MIN_ON_TIME) {
                    _burnerOn = false;
                    _burnerStopTime = now;
                    framOk(_storage->logEvent(rtstorage::EVENT_STATE_CHANGE, 0x0101), "logEvent");  // Burner off
                }
            }
...
void BoilerController::checkSafetyConditions() {
    ...
    if (_flowTemp > TEMP_OVERHEAT_THRESHOLD) {
        ...
        if (!(_activeErrors & ERROR_OVERHEAT)) {
            framOk(_storage->logCriticalError(ERROR_OVERHEAT, "Flow overheat", "Safety"), "logCriticalError");
        }
    }
    ...
    if (_outsideTemp < TEMP_FREEZE_THRESHOLD && _operatingMode == MODE_OFF) {
        ...
        if (!(_activeErrors & ERROR_FREEZE_RISK)) {
            framOk(_storage->logCriticalError(ERROR_FREEZE_RISK, "Freeze risk", "Safety"), "logCriticalError");
        }
    }
    ...
    if (_flowTemp < -50.0f || _flowTemp > 150.0f) {
        ...
        if (!(_activeErrors & ERROR_SENSOR_FAILURE)) {
            framOk(_storage->logError(ERROR_SENSOR_FAILURE, "Flow sensor fault", "Sensor"), "logError");
        }
    }
    ...
    if (newErrors != 0 && _lastErrorCode != newErrors) {
        ...
        framOk(_storage->incrementCounter(rtstorage::COUNTER_ERROR_COUNT), "incrementCounter");
    }
}
...
void BoilerController::updateOutputs() {
    ...
    if (millis() - lastRuntimeUpdate > 1000) {  // Every second
        ...
        if (_burnerOn) {
            ...
            framOk(_storage->updateRuntimeHours(rtstorage::RUNTIME_BURNER, hours + (1.0f / 3600.0f)), "updateRuntimeHours");
        }
    }
}
...
void BoilerController::logStateChange(uint8_t oldMode, uint8_t newMode) {
    framOk(_storage->logEvent(rtstorage::EVENT_STATE_CHANGE, (oldMode << 8) | newMode), "logEvent");
    
    // Update pump start counters
    if (newMode == MODE_HEATING || newMode == MODE_BOTH) {
        if (oldMode != MODE_HEATING && oldMode != MODE_BOTH) {
            framOk(_storage->incrementCounter(rtstorage::COUNTER_HEATING_PUMP_STARTS), "incrementCounter");
        }
    }
    
    if (newMode == MODE_HOT_WATER || newMode == MODE_BOTH) {
        if (oldMode != MODE_HOT_WATER && oldMode != MODE_BOTH) {
            framOk(_storage->incrementCounter(rtstorage::COUNTER_WATER_PUMP_STARTS), "incrementCounter");
        }
    }
}
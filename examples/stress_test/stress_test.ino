...
bool testPIDStates() {
    ...
    for (uint8_t i = 0; i < rtstorage::MAX_PID_CONTROLLERS; i++) {
        rtstorage::PIDState writeState = {
            ...
            .output = static_cast<float>(random(101)),
...
bool testConcurrentAccess() {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        framOk(fram.recordTemperature(random(8), 250 + random(100)), "recordTemperature");
        stats.totalWrites++;
        ...
        rtstorage::PIDState pid = {
            .integral = i * 1.5f,
            .lastError = i * 0.1f,
            .output = static_cast<float>(i % 100),
            .lastUpdateTime = millis()
        };
        framOk(fram.savePIDState(i % 4, pid), "savePIDState");
        stats.totalWrites++;
        
        // System 4: Log event
        framOk(fram.logEvent(rtstorage::EVENT_STATE_CHANGE, i), "logEvent");
        stats.totalWrites++;
...
bool testPowerLossRecovery() {
    ...
    // Save some counters
    framOk(fram.setCounter(rtstorage::COUNTER_ERROR_COUNT, 0xDEADBEEF), "setCounter");
    stats.totalWrites++;

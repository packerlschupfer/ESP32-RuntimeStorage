...
void loop() {
    ...
    if (millis() - lastErrorTime > 5000) {
        ...
        switch (errorCount % 4) {
            case 0:
                framOk(storage.logError(ERR_SENSOR_TIMEOUT, "DS18B20 timeout", "Sensor 1"), "logError");
                Serial.println(F("Logged sensor timeout error"));
                break;
                
            case 1:
                framOk(storage.logError(ERR_TEMP_OUT_OF_RANGE, "Temp: 125.5C", "Boiler"), "logError");
                Serial.println(F("Logged temperature range error"));
                break;
                
            case 2:
                framOk(storage.logError(ERR_COMM_FAILURE, "ModBus CRC fail", "VFD"), "logError");
                Serial.println(F("Logged communication error"));
                break;
                
            case 3:
                // Every 4th error is critical
                framOk(storage.logCriticalError(ERR_OVERHEAT, "Shutdown at 95C", "Safety"), "logCriticalError");
...
void demonstrateErrorLogging() {
    ...
    // Log various errors
    framOk(storage.logError(ERR_SENSOR_TIMEOUT, "No response from sensor"), "logError");
    Serial.println(F("Logged: Sensor timeout"));
    delay(100);
    
    framOk(storage.logError(ERR_TEMP_OUT_OF_RANGE, "Temp reading: -127.0"), "logError");
    Serial.println(F("Logged: Temperature out of range"));
    delay(100);
    
    framOk(storage.logCriticalError(ERR_WATCHDOG_RESET, "WDT reset detected", "System"), "logCriticalError");
    Serial.println(F("Logged: CRITICAL - Watchdog reset"));
    delay(100);
    
    framOk(storage.logError(ERR_COMM_FAILURE, "I2C bus stuck"), "logError");
    Serial.println(F("Logged: Communication failure"));
    delay(100);
    
    framOk(storage.logCriticalError(ERR_POWER_LOSS, "Brownout detected", "Power"), "logCriticalError");

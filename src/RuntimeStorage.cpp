/*
 * RuntimeStorage.cpp - part of the ESP32-RuntimeStorage library
 *
...
bool RuntimeStorage::i2cWrite(uint16_t address, const uint8_t* data, size_t length) {
    ...
    // F42: split long writes into SEPARATE I2C transactions, each carrying its
    // own 2-byte address header, and verify the bytes were actually buffered.
    // Previously every chunk was write()n into a SINGLE transaction: the ESP32
    // TwoWire TX buffer silently drops bytes past its capacity, so any write
    // longer than the buffer (~126 B) was truncated while endTransmission()
    // still returned 0 (success). That made format() only erase the first
    // ~126 B of each 256 B block while reporting success.
    const size_t MAX_PAYLOAD = 30;  // 2 addr bytes + 30 payload fits a 32 B Wire buffer
    for (uint8_t retry = 0; retry < MAX_RETRIES; retry++) {
        bool ok = true;
        size_t written = 0;
        while (written < length) {
            size_t chunkSize = min(length - written, MAX_PAYLOAD);
            uint16_t chunkAddr = (uint16_t)(address + written);

            _wire->beginTransmission(_i2cAddress);
            _wire->write((uint8_t)(chunkAddr >> 8));
            _wire->write((uint8_t)(chunkAddr & 0xFF));
            size_t buffered = _wire->write(data + written, chunkSize);
            bool txOk = (_wire->endTransmission() == 0);

            if (buffered != chunkSize || !txOk) {
                ok = false;
    ...
    for (uint8_t retry = 0; retry < MAX_RETRIES; retry++) {
        ...
        if (_wire->endTransmission() == 0) {
            ...
            while (read < length) {
                ...
                // F43: if requestFrom returned nothing (FRAM NACK / SDA stuck low
                // on the shared I2C bus), break instead of spinning forever with
                // the FRAM mutex held until the task watchdog reboots the ESP32.
                // A no-progress read falls through to the outer MAX_RETRIES loop
                // and ultimately fails, degrading to "no persistence" as intended.
                if (available == 0) {
                    break;
                }

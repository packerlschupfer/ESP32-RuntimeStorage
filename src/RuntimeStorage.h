/*
 * RuntimeStorage.h - part of the ESP32-RuntimeStorage library
 *
...
public:
    ...
    // F41: expose the internal I2C mutex so another driver sharing the same
    // physical FRAM/I2C bus (e.g. the firmware's CriticalDataStorage, which does
    // raw Wire transactions to the same chip) can serialize against RuntimeStorage
    // and avoid interleaving a two-phase (set-address, then read) sequence that
    // moves the FRAM address latch mid-read. Returns nullptr if mutex protection
    // is compiled out. Take/give with the standard FreeRTOS semaphore API.
    [[nodiscard]] SemaphoreHandle_t getBusMutex() const noexcept {
#if USE_MUTEX_PROTECTION
        return _mutex;
#else
        return nullptr;
